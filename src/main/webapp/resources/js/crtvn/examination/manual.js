$(function(){
	...
	function profession() {		
		$('#aprofession').combobox({
			url : "profession/professionSelect?collegeId=" + $('#collegeId').val(),
			valueField : 'id',
			textField : 'text',
			width : 165,
	...
	function aloadCourse() {
		...
		$('#acourse').combobox(
				{
				url : "course/courseSelect?professionId="
				...
				onChange:function(){
					...
					if(id != 0){
						$.ajax({
							type : "post",
							url : "student/studentCountbyCourse",
							data : {"courseId":id},
							dataType : "json",
							success : function(data) {
								count = data.count;
	...
	function loadClassroom(){
		...
		$('#aclassroom').combogrid({  
			url:'classroom/classroomDataGrid',
		    panelWidth:700,   
		    panelHeight: 370, 
		    idField:'classroomId',   
		    textField:'code', 
	...
	function loadTeacher(){
		...
		$('#ateacherCode').combogrid({  
	        ...
	        multiple: true,
	        editable: false,
	        queryParams : { "collegeId":$('#college').combobox('getValue'),
	        				"mark":1
	        				},
		    columns:[[ 
		            {field:"ck",width:50,checkbox:true,}, 
	...
	$('#aetime').combobox(
			{
			url : "timeDate/timeSelect?mark=" + 2,
			valueField : 'id',
			textField : 'text',
			width : 300,
			onLoadSuccess : function() { // 加载完成后,设置选中第一项
	...
	function qloadTeacher1() {
		...
		var urlStr = "examination/examTeacherSelect?mark=" + 2 +"&status=" + 1 
					+"&professionId=" + $('#qprofession').combobox('getValue');
		qloadTeacher("qteacherCode1",urlStr);
	};
	
	function qloadTeacher2() {
		var urlStr = "examination/examTeacherSelect?mark=" + 2 +"&status=" + 2
					+"&professionId=" + $('#qprofession').combobox('getValue');
	...
	$("#manualDataGrid").datagrid({
		url : "examination/examinationDataGrid",
		title : "考场信息列表",
		width : bodyWidth - 20,
	...
	$('#downloadE').click(function(){	
		$.messager.confirm('提示','是否下载查询的考场信息？',function(r){   
			if (r){   
			  ...
			  window.location.href="examination/exportExcelExamination?professionId="
	...
	$('#downloade').click(function(){	
		$.messager.confirm('提示','是否下载查询到的考场对应的考试信息？',function(r){   
			if (r){   
			  var params = setParams() ;
			  window.location.href="examination/exportExcelExamn?professionId="
				  +params.professionId+"&courseId="+params.courseId
				  +"&teacherId1="+params.teacherId1+"&teacherId2="+params.teacherId2
				  +"&mark="+params.mark;
			}   
