$(function() {
				 ...
				 formatter: function(value,row,index){					 
					 	if(row.thisId == 0 || row.thisId == null || row.thisId == ''){
					 		return "<a>详情</a>"
					 		 ...
					 		 return '<a style="color:blue" href="teacher/teacherDetails?collegeId='
	...
	function profession() {
		...
		// 专业信息下拉框
		$('#profession').combobox({
			url : "profession/professionSelect?collegeId=" + collegeId,
			valueField : 'id',
			textField : 'text',
			width : 165,
			onChange : function() {
	...
	function teacher() {
		...
		$('#teacherCode').combobox(
				{
					url : "teacher/teacherSelect?collegeId=" + collegeId
	...
	function course(){
		...
		if (professionId == '') {
		  ...
		  //课程代号下拉框
	   	 $('#courseCode').combobox({
 		 	url: "course/courseSelect?professionId=" + professionId,                        
 		 	valueField: 'id',
            textField: 'text',
            width:165,
