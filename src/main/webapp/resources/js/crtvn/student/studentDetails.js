$(function(){
	 ...
	 $('#testSubjects').combobox({
		 url: "student/studentExamCourseYesSelect?mark=2&studentId="+$('#studentId').val(),                        
         valueField: 'id',
         textField: 'text',
         width:165,
	 ...
	 $('#noTestSubjects').combobox({
		 url: "student/studentExamCourseNoSelect?mark=2&studentId="+$('#studentId').val(),                        
         valueField: 'id',
         textField: 'text',
         width:165,
         onLoadSuccess: function () { //加载完成后,设置选中第一项
	...
	//考试状态下拉框
	 ...
	 $('#keep').click(function(){
		 ...
		 if(flag){
			 $.ajax({
	             type: "post",
	             url: "student/studentExamCourseStatus",
	             data:  param,
	             dataType: "json",
	             success: function(data){
	            	var msg = "失败";
